...
def extract_task(**kwargs):
    """
    1) Creates (or gets) a unique GCS bucket
    2) Extracts & partitions the data
    3) XCom-pushes the bucket name so the next task can use it
    """
    ...
    # Make a unique bucket name each time this task runs
...
def create_external_table_task(**kwargs):
    ...
    2) Create an external BQ table with hive partitioning
    """
    from scripts.bikeshare import (
        creds,
        PROJECT_ID,
    ...
    ti = kwargs['ti']
    unique_bucket_name = ti.xcom_pull(
        key='bucket_name',
        task_ids='extract_bikeshare_data'
    )

    # Must already have a dataset named "my_dataset" in your GCP project
...
default_args = {
    ...
    'start_date': datetime(2023, 1, 1),  # backfill start
    'retries': 1,
